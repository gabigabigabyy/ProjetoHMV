...
@Component({
  selector: 'app-detalhamento-total',
  templateUrl: './detalhamento-total.component.html',
  styleUrls: ['./detalhamento-total.component.css'],
  providers: [
    ConverterDataPipe
  ]
})
export class DetalhamentoTotalComponent implements OnInit {
  ...
  ngOnInit() {
    this.activeRoute.params
      .subscribe(params => {
        this.cdLocal = (params['cdLocal']);
      });

    this.loadDataSourceTeste();
    this.getContas();
  }

  private loadDataSourceTeste() {
    this.showLoading();
  }

  private showLoading() {
    this.loading = true;
    var totalContas = this.service.getTotalContas().subscribe(list => {
  ...
  getContas() {
    var contas = this.service.getTotalContas().subscribe(contas => {
